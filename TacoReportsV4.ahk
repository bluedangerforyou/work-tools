...
Gui, Add, DropDownList, x20 y440 w475 vListSMARTREPORTS gOnselect, Team availability|Prep guide|In-store labor variance|TLD fix for Prep guide errors|
...
Gui, Add, DropDownList, x20 y650 w475 vListPOScmd gOnselect, Runas|Runas CMD|Runas Explorer|CHKDSK DEFRAG|POS Remote Reboot|Remote List Tasks|Remote Kill Task|Device Manager|SQL Server Mgmt Studio|UpdateLinks|RegMerge|HDD Wrapper Recovery|Create End of Day backups|Biometric Switch
...
if (ListSMARTREPORTS = "TLD fix for Prep guide errors")
{
	clipboard = tldfix.sh
}
...
if (ListPOScmd = "HDD Wrapper Recovery")
{
	clipboard = c:\windows\options\bin\hddrecovery_wrapper.exe
}
if (ListPOScmd = "Create End of Day backups")
{
	clipboard = c:\windows\options\bin\eodbackup.exe
}
return
if (ListPOScmd = "Biometric Switch")
{
	clipboard = c:\windows\options\bin\biometricswitch.exe
}
